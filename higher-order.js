...
/*
  ...
  (given to you as a tax rate,
     hint: you'll need to do some multiplication). Your answer should be an array of 
     numbers, one total for each order.
*/

let orderTotals = order.map(item => {
  let {price, tax} = item
...
let asdf = [];
let compilation = purchases.map(function(element){
  if (element.owner === "Bob")
    asdf.push(element.price)
    })

console.log(asdf)

let bobsTotal = asdf.reduce(function(acc, curr){
    return acc + curr})
...
// let bobsTotal = compilation.reduce(acc, curr)
//     return acc + curr
